...
%% alternative to ceph_w.m with different approach to parse the pg status (no more than 99999 pgs)
%% jun.xu@wdc.com
...
while ischar(tline)
    ...
    if (size(idx0,2)>1) & (size(idx1,2)>=1) & (size(idx6,2)>=1)
        ...
        for j=1:pg_status_num
            idx3=strfind(tline,pg_status{j});
            if ~isempty(idx3)
                ...
                for ll=1:size(idx3,2)
                    % right string shall not have "+" in front or end.
                    if (tline(idx3(ll)-1)=='+')
                        find_right_str=0;
                        continue
                    end                    
                    
                    if (tline(idx3(ll)+size(pg_status{j},2))=='+')
                        find_right_str=0;
                        continue
                    end
                                      
                    % if no "+" before or after the status string, then
                    % it is the right one
                    % assume that the total PG number is less than 99999
                    
                    temp_str=tline(idx3(ll)-6:idx3(ll));
                    num_str_idx=regexp(temp_str,['\d']);
                    temp_value=sscanf(temp_str(num_str_idx),'%d');                             
